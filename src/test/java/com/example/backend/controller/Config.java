		...
		http
				...
				.exceptionHandling(ex -> ex
						.authenticationEntryPoint((req, res, e) ->
								writeProblem(res, om, req.getRequestURI(), HttpStatus.UNAUTHORIZED))
						.accessDeniedHandler((req, res, e) ->
								writeProblem(res, om, req.getRequestURI(), HttpStatus.FORBIDDEN))
				);
		return http.build();
	}
	
	private static void writeProblem(HttpServletResponse res, ObjectMapper om,
									 String uri, HttpStatus status) throws IOException {
...
class TestErrorAdvice {
	...
	handleValidation(org.springframework.web.bind.MethodArgumentNotValidException ex,
		...
		pd.setStatus(422);
		pd.setTitle("Unprocessable Content");
	...
	public org.springframework.http.ProblemDetail handleNotFound(
			com.example.backend.common.error.NotFoundException ex,
			jakarta.servlet.http.HttpServletRequest req) {
